$(document).ready(function() {
    ...
    function bmiCalc() {
        $("#submitBTN").on("click", function(event) {
            ...
            $.ajax(settings).done(function(response) {
                ...
                $("#results").append("<hr>");
                $("#results").append(
                    '<p class="lead"><b>BMI: </b>' + bmi.toFixed(2) + "</p>"
                );
                $("#results").append(
                    '<p class="lead"><b>Overall Health: </b>' + health + "</p>"
                );
                $("#results").append(
                    '<p class="lead"><b>Healthy BMI Range: </b>' + range + "</p>"
                );
            });
        });
    }
    ...
    function calorieCalc() {
        $("#submitBTN").on("click", function(event) {
            ...
            $.ajax(settings).done(function(response) {
                ...
                $("#cal-results").append(
                    '<p class="lead"><b>Calories: </b>' + calories + "</p>"
                );
            });
        });
    }
    ...
    refreshTimer = setInterval(function() {
        ...
        percent = Math.round(
            ((hours + minutes + 1) / (parseInt(start.type) * 100)) * 100
        );
    ...
    function NYTquery() {
        ...
        let queryURL =
            "https://api.nytimes.com/svc/search/v2/articlesearch.json?q=Health%20%26%20Fitness&api-key=tZIiJgEGHbCV0KpFDgTXFHQH8FF0pLDt";
        ...
        $.ajax({
            ...
            .then(function(response) {
                ...
                $("#articleIMG1").append(
                    '<a href="' +
                    response.response.docs[2].web_url +
                    '" target="blank"><img src="https://static01.nyt.com/' +
                    response.response.docs[2].multimedia[19].url +
                    '"></a>'
                );
                ...
                $("#articleHead1").append(
                    ...
                    response.response.docs[2].web_url +
                    '" target="blank"><h2>' +
                    response.response.docs[2].headline.main +
                    "</h2></a>"
                );
                $("#articleHead1").append(
                    "<p><strong>" + response.response.docs[2].snippet + "</strong></p>"
                );
                $("#articleHead1").append(
                    "<p>" + response.response.docs[2].pub_date + "</p>"
                );

                $("#articleIMG2").append(
                    '<a href="' +
                    response.response.docs[3].web_url +
                    '" target="blank"><img src="https://static01.nyt.com/' +
                    response.response.docs[3].multimedia[19].url +
                    '"></a>'
                );
                ...
                $("#articleHead2").append(
                    ...
                    response.response.docs[3].web_url +
                    '" target="blank"><h2>' +
                    response.response.docs[3].headline.main +
                    "</h2></a>"
                );
                $("#articleHead2").append(
                    "<p><strong>" + response.response.docs[3].snippet + "</strong></p>"
                );
                $("#articleHead2").append(
                    "<p>" + response.response.docs[3].pub_date + "</p>"
                );

                $("#articleIMG3").append(
                    '<a href="' +
                    response.response.docs[5].web_url +
                    '" target="blank"><img src="https://static01.nyt.com/' +
                    response.response.docs[5].multimedia[19].url +
                    '"></a>'
                );
                ...
                $("#articleHead3").append(
                    ...
                    response.response.docs[5].web_url +
                    '" target="blank"><h2>' +
                    response.response.docs[5].headline.main +
                    "</h2></a>"
                );
                $("#articleHead3").append(
                    "<p><strong>" + response.response.docs[5].snippet + "</strong></p>"
                );
                $("#articleHead3").append(
                    "<p>" + response.response.docs[5].pub_date + "</p>"
                );
            });
    }
